...
export default class ExpressionParser extends UtilParser {
  ...
  parseSubscripts(expr, nocalls) {
    if (this.match(".")) {
      ...
      let quasi = this.parseTemplate()
      return this.parseSubscripts(new Node.TaggedTemplateExpression(expr, quasi))
    }
  ...
  parseAtomicExpression() {
    ...
    switch (token.type) {
      ...
      case TokenTypes.TemplateLiteral:
  ...
  parseTemplate() {
    ...
    while (!ele.tail) {
      ...
      ele = this.parseTemplateElement()
      quasis.push(ele)
    }
    this.expect("`")
    this.context.inTemplate = false
  ...
  parseTemplateElement() {
    ...
    if (this.lookahead.type !== TokenTypes.Punctuator) {
      let token = this.nextToken()
      value = { raw: token.raw, cooked: token.value }
    }
    let tail = this.match("`")
